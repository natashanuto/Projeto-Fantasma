...
posicao_time <- times %>%
  mutate(
    Posição = case_when(
      Pos %>% str_detect("C") ~ "C(Pivô)",
      Pos %>% str_detect("PF") ~ "PF(Ala-Pivô)",
      Pos %>% str_detect("SF") ~ "SF(Ala)",
      Pos %>% str_detect("SG") ~ "SG(Ala-Armador)",
      Pos %>% str_detect("PG") ~ "PG(Armador)",
...
ggsave("exploratoria5.png", width = 158, height = 93, units = "mm")
...
total_turnover <- aggregate(TOV ~ Pos, data = basquete, sum)

ggplot(total_turnover, aes(x = Pos, y = TOV , fill = Pos)) +
...
ggsave("analise5.png", width = 158, height = 93, units = "mm")
